...
- **Any real product/catalog data persisted between sessions** — pages render correctly with empty results; Milestones 9 through 13's own visual validation each used temporary QA-only products, created and deleted (or, for Milestone 13's wine product, unpublished after a real order's inventory reservation blocked deletion) via the Admin API specifically for that purpose — see this README's own Milestone sections and `DECISION_LOG.md`.
