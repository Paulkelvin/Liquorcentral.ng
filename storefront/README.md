...
- **Any product/catalog data** — no products have been seeded; pages render correctly with empty results (Milestones 9 and 10's own visual validation each used temporary QA-only products, created and deleted via the Admin API specifically for that purpose — see this README's Milestone 9/10 sections and `DECISION_LOG.md`).
