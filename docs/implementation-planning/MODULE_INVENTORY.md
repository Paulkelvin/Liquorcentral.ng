...
- **Which are custom?** `wine-details`, the Product Relationship Module, `food-details`, and delivery-slot scheduling (all four architecture approved; `wine-details` and `food-details` are also **implemented in code**, Engineering Milestones 2 and 3); and Saved-for-Later (not yet scoped).
