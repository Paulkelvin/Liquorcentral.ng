...
See `PROJECT_STATUS.md` for the current, consolidated list. As of this version: Phase 0 (including 0b) is complete with no open items; `01_NAVIGATION_SPECIFICATION.md`, `02_HOMEPAGE_SPECIFICATION.md`, `03_SEARCH_SPECIFICATION.md`, `04_PRODUCT_LISTING_SPECIFICATION.md`, `05_PRODUCT_DETAILS_SPECIFICATION.md`, `06_CART_SPECIFICATION.md`, and `07_CHECKOUT_SPECIFICATION.md` are all Approved — Frozen with no open items on any of the seven documents themselves, though `01`, `03`, `06`, and `07` each surfaced their own tracked open items — the exact Wine & Spirits mega-menu category grouping (navigation); Meilisearch's still-pending formal sign-off plus the unscoped "pairs with" cross-sell relationship (search, now depended on by eight specifications in total); and, confirmed rather than newly discovered at the checkout level, five open delivery-related business decisions (Wine & Spirits delivery mechanism, Lagos delivery-area definition, delivery-slot parameters, cash-on-delivery, hard age-recheck at order confirmation) plus one new recommendation requiring scoping confirmation (Saved-for-Later, `06_CART_SPECIFICATION.md` §14/§26) — all logged in `PROJECT_STATUS.md`. `08_CUSTOMER_ACCOUNT_SPECIFICATION.md` is now drafted (v0.1, In Progress) and surfaces two further open items of its own — data-retention/NDPR-compliance specifics and the account deletion-vs-deactivation policy — plus the notification-channel decision as a shared dependency, awaiting Paul's review. Phase 0d (Product Specifications) otherwise awaits Paul's sequencing direction for specs `09`–`11`, none of which begins without his explicit instruction. The payment provider choice (`MEDUSA_EXTENSIONS.md` #4) remains the most launch-critical open decision, blocking both backend Phase 1 and final implementation of `07_CHECKOUT_SPECIFICATION.md`'s Payment Behaviour and Payment State Behaviour sections. None of these block each other.
