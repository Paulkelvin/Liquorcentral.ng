...
- `02_HOMEPAGE_SPECIFICATION.md` — **fully drafted (v0.1)**, all 25 required sections, status **Under Review**, awaiting Paul's approval — the only specification still pending a decision.
...
See `PROJECT_STATUS.md` for the current, consolidated list. As of this version: Phase 0 (including 0b) is complete with no open items; `01_NAVIGATION_SPECIFICATION.md` and `03_SEARCH_SPECIFICATION.md` are both Approved — Frozen with no open items on either document itself, though each surfaced its own tracked open item — the exact Wine & Spirits mega-menu category grouping (navigation), and Meilisearch's still-pending formal sign-off plus the unscoped "pairs with" cross-sell relationship (search), both in `PROJECT_STATUS.md`. Phase 0d (Product Specifications) otherwise has only `02_HOMEPAGE_SPECIFICATION.md` awaiting review (specs `04`–`11` await Paul's sequencing direction), and specs `07`/`10` will need the payment-provider and courier/notification decisions resolved before they can be developed in detail. The payment provider choice (`MEDUSA_EXTENSIONS.md` #4) remains the most launch-critical open decision blocking backend Phase 1. None of these block each other.
