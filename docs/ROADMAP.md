...
### Phase 0f — Implementation Planning — In progress

**Naming note:** referred to elsewhere in `/docs` (`AI_HANDOFF.md`, `IMPLEMENTATION_PLANNING.md`) as "Phase 2," in the sense of following Phase 0 (Brand & Design) and Phase 0d (Product Specifications) in the documentation/planning lineage. It is numbered "Phase 0f" here specifically to avoid colliding with this document's own, unrelated "Phase 2 — Product data foundation" below (a backend-build milestone within the Phase 1–9 numbering track) — the identical reasoning this document already gives for the Phase 1 naming collision at the top of this file.

- **`docs/IMPLEMENTATION_PLANNING.md` created and approved at v1.0 (2026-07-19)** — the governing document for how Phase 2's engineering/implementation documents are organized, sequenced, owned, approved, versioned, and changed, in the same relationship to those future documents that `DOCUMENTATION_GOVERNANCE.md` holds over `/docs` as a whole.
- Defines 16 required implementation documents (`01_SYSTEM_ARCHITECTURE.md` through `16_TESTING_STRATEGY.md`), to live in a new `docs/implementation/` directory mirroring the `docs/specifications/` pattern — none created yet; none begins without Paul's explicit direction to start that specific one.
- Defines 16 architecture philosophies (Medusa, Next.js, backend, frontend, database, API, search, CMS, media, authentication, payment, security, performance, accessibility, testing, deployment) every future implementation document must follow.
- Surfaces two genuinely new open decisions while scoping the document list: a media/image-hosting provider (for `10_MEDIA_ARCHITECTURE.md`) and a hosting/infrastructure provider (for `14_DEPLOYMENT_ARCHITECTURE.md`) — neither ever named anywhere in `/docs` before this document. See `PROJECT_STATUS.md`.
- Can run in parallel with Phase 0c and Phase 1's backend work — it is documentation, not implementation, and does not block or get blocked by either, the identical treatment already given to Phase 0d above.
