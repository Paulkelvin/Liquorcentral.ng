...
- `07_CHECKOUT_SPECIFICATION.md` — **In Progress (v0.1)**, 2026-07-18, drafted in full (all 30 sections), awaiting Paul's review. Extends `06_CART_SPECIFICATION.md` directly — completes its Pricing Transparency table, resolves its deferred delivery-eligibility check, and carries its two-fulfillment-leg mixed-order model through to order confirmation.
- `08_CUSTOMER_ACCOUNT_SPECIFICATION.md`, `09_FOOD_ORDERING_SPECIFICATION.md`, `10_DELIVERY_SPECIFICATION.md`, `11_ADMIN_WORKFLOWS_SPECIFICATION.md` — remain **approved placeholders only** (Document Purpose, Scope, Dependencies, Planned Sections, Status = Not Started). Detailed content is intentionally not invented ahead of sequencing; each will be developed in its own turn once Paul directs.
...
See `PROJECT_STATUS.md` for the current, consolidated list. As of this version: Phase 0 (including 0b) is complete with no open items; `01_NAVIGATION_SPECIFICATION.md`, `02_HOMEPAGE_SPECIFICATION.md`, `03_SEARCH_SPECIFICATION.md`, `04_PRODUCT_LISTING_SPECIFICATION.md`, `05_PRODUCT_DETAILS_SPECIFICATION.md`, and `06_CART_SPECIFICATION.md` are all Approved — Frozen with no open items on any of the six documents themselves, though `01`, `03`, and `06` each surfaced their own tracked open items — the exact Wine & Spirits mega-menu category grouping (navigation); Meilisearch's still-pending formal sign-off plus the unscoped "pairs with" cross-sell relationship (search, now depended on by six specifications in total); and, from `06_CART_SPECIFICATION.md` §6, five open delivery-related business decisions (Wine & Spirits delivery mechanism, Lagos delivery-area definition, delivery-slot parameters, cash-on-delivery, hard age-recheck at order confirmation) plus one new recommendation requiring scoping confirmation (Saved-for-Later, §14/§26) — all logged in `PROJECT_STATUS.md`. `07_CHECKOUT_SPECIFICATION.md` is now drafted (v0.1, In Progress) and confirms, rather than newly discovers, the same five delivery-related decisions plus the payment-provider and notification-channel gaps as its own direct dependencies — awaiting Paul's review. Phase 0d (Product Specifications) otherwise awaits Paul's sequencing direction for specs `08`–`11`. The payment provider choice (`MEDUSA_EXTENSIONS.md` #4) remains the most launch-critical open decision, blocking both backend Phase 1 and `07_CHECKOUT_SPECIFICATION.md`'s final implementation. None of these block each other.
