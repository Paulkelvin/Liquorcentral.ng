...
See `PROJECT_STATUS.md` for the current, consolidated list. As of this version: Phase 0 (including 0b) is complete with no open items; `01_NAVIGATION_SPECIFICATION.md`, `02_HOMEPAGE_SPECIFICATION.md`, `03_SEARCH_SPECIFICATION.md`, and `04_PRODUCT_LISTING_SPECIFICATION.md` are all Approved — Frozen with no open items on any of the four documents themselves, though `01` and `03` each surfaced their own tracked open item — the exact Wine & Spirits mega-menu category grouping (navigation), and Meilisearch's still-pending formal sign-off plus the unscoped "pairs with" cross-sell relationship (search, now also depended on by listing cross-selling) — both in `PROJECT_STATUS.md`. Phase 0d (Product Specifications) otherwise awaits Paul's sequencing direction for specs `05`–`11`, and specs `07`/`10` will need the payment-provider and courier/notification decisions resolved before they can be developed in detail. The payment provider choice (`MEDUSA_EXTENSIONS.md` #4) remains the most launch-critical open decision blocking backend Phase 1. None of these block each other.
