...
- `06_CART_SPECIFICATION.md` — **In Progress (v0.1)**, 2026-07-18, fully drafted (all 30 required sections, including a deep-dive Mixed Cart Behaviour section and a numbered Cart Quality Checklist), awaiting Paul's review before it can be finalized to v1.0.
- `07_CHECKOUT_SPECIFICATION.md`, `08_CUSTOMER_ACCOUNT_SPECIFICATION.md`, `09_FOOD_ORDERING_SPECIFICATION.md`, `10_DELIVERY_SPECIFICATION.md`, `11_ADMIN_WORKFLOWS_SPECIFICATION.md` — remain **approved placeholders only** (Document Purpose, Scope, Dependencies, Planned Sections, Status = Not Started). Detailed content is intentionally not invented ahead of sequencing; each will be developed in its own turn once Paul directs. Per Paul's explicit instruction, `07_CHECKOUT_SPECIFICATION.md` does not begin until Cart has been reviewed and approved.
...
See `PROJECT_STATUS.md` for the current, consolidated list. As of this version: Phase 0 (including 0b) is complete with no open items; `01_NAVIGATION_SPECIFICATION.md`, `02_HOMEPAGE_SPECIFICATION.md`, `03_SEARCH_SPECIFICATION.md`, `04_PRODUCT_LISTING_SPECIFICATION.md`, and `05_PRODUCT_DETAILS_SPECIFICATION.md` are all Approved — Frozen with no open items on any of the five documents themselves, though `01` and `03` each surfaced their own tracked open item — the exact Wine & Spirits mega-menu category grouping (navigation), and Meilisearch's still-pending formal sign-off plus the unscoped "pairs with" cross-sell relationship (search, now depended on by six specifications in total, including product listing, product details, and cart cross-selling). `06_CART_SPECIFICATION.md` is fully drafted (v0.1, In Progress) and awaits Paul's review; it surfaced five of its own open delivery-related business decisions (§6: Wine & Spirits delivery mechanism, Lagos delivery-area definition, delivery-slot parameters, cash-on-delivery, hard age-recheck at order confirmation) and one new recommendation requiring scoping confirmation (Saved-for-Later, §14/§26) — all logged in `PROJECT_STATUS.md`. Phase 0d (Product Specifications) otherwise awaits Paul's review of Cart before `07_CHECKOUT_SPECIFICATION.md` begins, and specs `07`/`10` will need the payment-provider and courier/notification decisions resolved before they can be developed in detail. The payment provider choice (`MEDUSA_EXTENSIONS.md` #4) remains the most launch-critical open decision blocking backend Phase 1. None of these block each other.
