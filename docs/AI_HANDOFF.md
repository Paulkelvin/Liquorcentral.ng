...
| **Next.js** (Medusa's official DTC Starter, scaffolded at `storefront/` — Milestone 5) | Customer-facing storefront | Ships with cart, checkout, region/tax handling, and payment integration already wired against Medusa's API, avoiding a from-scratch reimplementation with no maintained upgrade path. SSR/SEO supports organic product discovery. Scaffolded and wired to the real backend, no LiquorCentral UI applied yet. See `TECH_STACK.md` and `storefront/README.md`. |
...
├── storefront/                        Milestone 5 — Medusa's official DTC Starter Next.js app, wired to backend/, own README.md
└── docs/
...
**`backend/` and `storefront/` are engineering output, not documentation** — see `backend/README.md` and `storefront/README.md` for what they actually contain; this file (`AI_HANDOFF.md`) only summarizes them. As of Milestone 5, `backend/` contains the wine-details, food-details, and delivery-slot custom modules (no custom API routes yet), and `storefront/` runs Medusa's official DTC Starter wired to the real backend, with no LiquorCentral UI applied yet.
...
**In Progress**
- None. Milestones 1 (Backend Foundation), 2 (`wine-details`), 3 (`food-details`), 4 (`delivery-slot`), and 5 (storefront scaffold) are complete. The local payment provider and notification provider tracks are blocked (see Blocked, below) rather than in progress; Wine & Spirits/Food Central fulfillment configuration is also blocked (Lagos-area definition, delivery-fee schedule).
...
- Component-level design work (`ROADMAP.md` Phase 0c) — including applying `DESIGN_SYSTEM.md`/`BRAND_IDENTITY.md` to the storefront scaffold, which still runs the vendored template's default UI.
...
- **The local payment provider module's provider choice, cash-on-delivery decision, and refund policy are all undecided** — recorded in `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` §4/§10/§16/§22, none resolved. Unlike the other modules above, this now directly blocks *code implementation itself* (Milestone 5, original numbering), not only Tier C/full-specification work — confirmed during that milestone's investigation (`DECISION_LOG.md`) that this module's actual mechanism *is* a specific PSP's API contract, so nothing further can be built until Paul chooses one.
- **The notification provider's Tier B document is still Draft, not Approved** — implementing against it would contradict `IMPLEMENTATION_PLANNING.md`'s Exit Criteria; blocks Milestone 6 (original numbering) until Paul approves or finalizes that document.
...
- **Phase 1 — Foundation (backend):** Stand up Medusa on Postgres + Redis — **✅ complete (Engineering Milestone 1)**; storefront scaffold — **✅ complete (Engineering Milestone 5)**, see `backend/README.md` and `storefront/README.md`. Remaining, both blocked on open business decisions: connect a real payment provider, and ship Wine & Spirits' full end-to-end buy flow (the storefront exists but has no catalog data or working payment yet).
...
Planning (Phases 0–2) is complete. Engineering proceeds **autonomously**, per Paul's explicit authorization (see Section 1): **Milestone 1 — Backend Foundation**, **Milestone 2 — the `wine-details` module**, **Milestone 3 — the `food-details` module**, **Milestone 4 — the `delivery-slot` module**, and **Milestone 5 — the storefront scaffold** are all complete (2026-07-19) — see `backend/README.md`, `storefront/README.md`, each module's own README, and `DECISION_LOG.md` for full detail:

1. **The local payment provider module (original priority #2) and notification provider module (original priority #3) are both confirmed blocked, not simply "not yet started."** Investigated directly during Milestone 5's planning: the payment provider's actual mechanism *is* a specific Nigerian PSP's API contract — there is no PSP-agnostic behavior to build independently of *which* PSP, unlike delivery-slot's mechanism/parameter split — so nothing further can be built until Paul chooses a provider. The notification provider's Tier B document is still **Draft**, not Approved; implementing against it would contradict `IMPLEMENTATION_PLANNING.md` §10's Exit Criteria. Both remain flagged stop conditions, not resolved by inventing a provider choice or a document approval.
2. **Food Central's Lagos-scoped fulfillment configuration, and Wine & Spirits' nationwide fulfillment configuration, were also investigated and found blocked** — the former on the exact Lagos delivery-area definition, the latter on the delivery-fee schedule (a Shipping Option needs a real price). Presented to Paul, who directed proceeding to the storefront scaffold instead — explicitly authorized to "begin now" per `IMPLEMENTATION_READINESS_REPORT.md` §5.
3. **With the storefront scaffold complete, the next unblocked engineering work is applying `DESIGN_SYSTEM.md`/`BRAND_IDENTITY.md` to it** (Phase 0c, component specification) — or any further business decision Paul supplies unblocking payment, fulfillment, or notification work. Neither has been started or directed yet.
4. **Paul to supply the business decisions `IMPLEMENTATION_READINESS_REPORT.md` §3 names** — most urgently the payment-provider selection and cash-on-delivery policy (`MEDUSA_EXTENSIONS.md` #4), the project's sole confirmed launch-blocking decision, now also the direct blocker on further backend code. Both attribute modules' field lists (`MEDUSA_EXTENSIONS.md` #1, #2) directly block full catalog population; `food-details`' allergen-accuracy-ownership question is safety-critical. `delivery-slot`'s operational parameters (slot length, cutoff, capacity), the exact Lagos delivery-area definition, and the delivery-fee schedule now directly block further fulfillment/scheduling work. The exact Nigerian VAT/tax rate remains an open item.
...
| Last Updated | 2026-07-19 |
| Project Phase | Phase 0 complete and frozen; Phase 1 — Product Specifications complete (all 11 frozen); Phase 2 — Implementation Planning complete and formally concluded; **Engineering begun — Milestone 1 (Backend Foundation), Milestone 2 (`wine-details` module), and Milestone 3 (`food-details` module) complete** |
| Next Planned Milestone | None unblocked without new direction from Paul — every remaining backend track (payment provider, notification provider, Food Central/Wine & Spirits fulfillment) is confirmed blocked on an open business decision; Phase 0c (applying `DESIGN_SYSTEM.md` to the storefront scaffold) is the only unblocked next step, and has not been directed yet — see Section 10 |
