...
└── docs/
    ...
    │   └── MODULE_INVENTORY.md                        Approved, living, v1.6 — index of every module
...
- `docs/implementation-planning/MODULE_INVENTORY.md` — living, approved 2026-07-18, corrected to v1.6 (2026-07-19). Single-page index of every module/integration/capability identified anywhere in `/docs`, with type, status, dependencies, and launch-criticality.
...
- No Product Specification placeholders remain — all 11 are frozen. Beyond Tier A (Approved) and four Tier B modules Approved (see Completed, above), no further Phase 2 implementation-planning document (per `IMPLEMENTATION_PLANNING.md` §4) has been started; none begins without Paul's explicit direction to begin that specific document.
...
**In Progress**
- None. All four Tier B documents drafted so far are Approved. No further Tier B document begins until Paul approves one.

**Not Started**
- Any implementation code, UI design, or wireframes.
- Every Phase 2 (Implementation Planning) Tier C–F document, and every Tier B document beyond the four approved so far, per `IMPLEMENTATION_PLANNING.md` §4 — none begins without Paul's explicit direction to begin that specific document.
...
- **Phase 0f — Implementation Planning (Paul's "Phase 2"): Tier A complete; four Tier B modules Approved.** `IMPLEMENTATION_PLANNING.md` v1.0 defines the six-tier structure; `TIER_A_FOUNDATIONAL_RECONCILIATION.md` (v1.3), `TIER_B_PRODUCT_RELATIONSHIP_MODULE.md`, `TIER_B_WINE_ATTRIBUTES_MODULE.md`, `TIER_B_FOOD_ATTRIBUTES_MODULE.md`, and `TIER_B_DELIVERY_SLOT_MODULE.md` are all Approved at v1.0; `MODULE_INVENTORY.md` (v1.6) is live; no further Tier B module begins without Paul's explicit direction.
...
Phase 1 — Product Specifications is complete (all 11 frozen), Phase 2's Tier A (Foundational Reconciliation) is complete and Approved (v1.3), and all four Tier B documents drafted so far — the Product Relationship Module, the Wine & Spirits Attributes Module, the Food Attributes Module, and the Delivery-Slot Module — are Approved at v1.0:

1. **Paul to direct which Tier B module begins next** — see `docs/implementation-planning/MODULE_INVENTORY.md` for the remaining candidates (the local payment provider, the notification provider, Saved-for-Later); nothing begins without his direction.
...
3. **Two architectural questions from `TIER_B_DELIVERY_SLOT_MODULE.md` remain open**, independent of which module begins next: the pickup-slot mechanism boundary (§5, §19); and the kitchen-capacity-vs-rider-capacity reconciliation question (§6, §18).
...
| Last Updated | 2026-07-19 |
| Project Phase | Phase 0 complete and frozen; Phase 1 — Product Specifications complete (all 11 frozen); Phase 2 — Implementation Planning's Tier A Approved (v1.3), four Tier B modules (Product Relationship Module, Wine & Spirits Attributes Module, Food Attributes Module, Delivery-Slot Module) Approved |
