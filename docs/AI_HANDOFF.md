...
└── docs/
    ...
    │   ├── TIER_A_FOUNDATIONAL_RECONCILIATION.md      Approved, v1.4 — complete
    ...
    │   ├── TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md    Draft, v1.0 — fifth Tier B module, awaiting review
    │   └── MODULE_INVENTORY.md                        Approved, living, v1.7 — index of every module
...
`/docs` contains 24 documents plus the 11-file `specifications/` directory (35 files total). `docs/README.md` is the detailed document map with cross-links and the document-status convention; `docs/DOCUMENTATION_GOVERNANCE.md` is the governing standard behind that convention (statuses, change rules, cross-reference rules, versioning, contributor rules); this section summarizes current status by category.
...
- `docs/implementation-planning/MODULE_INVENTORY.md` — living, approved 2026-07-18, corrected to v1.7 (2026-07-19). Single-page index of every module/integration/capability identified anywhere in `/docs`, with type, status, dependencies, and launch-criticality.
...
### Draft (awaiting Paul's review before any refinement/freeze pass)

- `docs/implementation-planning/TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` — the fifth Tier B module, drafted 2026-07-19. Purely architectural, provider-agnostic definition of the local payment provider module — responsibilities, non-responsibilities, the payment-method/mechanism/status/refund-policy boundary, payment lifecycle and payment-state responsibilities, provider abstraction responsibilities, ownership, its interaction with Medusa's native Payment module, and integration points across Checkout/Customer Account/Order Management/Admin Workflows, plus a dedicated Refund Workflow boundary section and confirmed non-integrations. Verified its dependency count (`07` only) requires no correction. Surfaces one newly-discovered documentation gap: `11_ADMIN_WORKFLOWS_SPECIFICATION.md` §11 names a refund workflow without naming this module as its mechanism, recorded not resolved. No specific payment provider is named anywhere. No database, API, or code content.

### In Progress

- None. No Product Specification is in progress. `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` is Draft, not In Progress — it awaits Paul's review, not further authoring.

### Not Started

- No Product Specification placeholders remain — all 11 are frozen. Beyond Tier A (Approved), four Tier B modules Approved, and a fifth Tier B module Drafted (see Completed/Draft, above), no further Phase 2 implementation-planning document (per `IMPLEMENTATION_PLANNING.md` §4) has been started; none begins without Paul's explicit direction to begin that specific document.
...
- **Fifth Tier B module — Local Payment Provider Module — drafted, Draft (2026-07-19)** — `docs/implementation-planning/TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` (v1.0, status Draft), drafted after re-applying the six standing selection criteria against every remaining candidate (the local payment provider, the notification provider, Saved-for-Later): the local payment provider wins on launch criticality (the project's sole confirmed launch-blocking decision) and architectural centrality (no order, of either catalog, can be placed without it), despite the lowest raw dependency count of any Tier B module drafted so far. Verified this module's dependency count directly against every specification's own Backend Requirements table and **found it requires no correction** — `07_CHECKOUT_SPECIFICATION.md` remains the sole genuine dependent, the first Tier B module whose baseline needed no swap-or-add adjustment. Surfaced one newly-discovered documentation gap: `11_ADMIN_WORKFLOWS_SPECIFICATION.md` §11 names a staff-facing refund workflow without naming this module as its underlying mechanism — recorded as a completeness recommendation, not resolved. Purely architectural and deliberately provider-agnostic throughout — no payment service provider is named or chosen anywhere, preserving the boundary between this Tier B document and a future Tier D provider-integration document. Defines the module's responsibilities and non-responsibilities (does not choose a provider, does not decide cash-on-delivery, does not perform reconciliation/accounting, does not decide refund policy); a dedicated boundary distinguishing payment method selection, this module's own mechanism, payment status on the Order, and refund policy as four separate concepts; payment lifecycle and payment-state responsibilities (reusing `07_CHECKOUT_SPECIFICATION.md`'s own Payment State Behaviour vocabulary directly); provider abstraction responsibilities; a two-way ownership split (engineering architecture / Paul's provider-choice and COD-policy approval); its interaction with Medusa's native Payment module; integration points across Checkout (the sole directly-evidenced consumer), Customer Account (read-only, mediated through the native Order module), Order Management (a module-to-module relationship), and Admin Workflows; a dedicated Integration with Refund Workflow section stating the architectural boundary explicitly (this module executes an authorized refund, it never decides policy); and a consolidated non-integrations section. Status intentionally **Draft**, awaiting Paul's review before any refinement pass or approval, per his instruction not to begin the refinement/freeze pass until explicitly approved.
...
**In Progress**
- None. `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` is Draft, awaiting Paul's review — not further authoring. No further Tier B document begins until Paul approves one.

**Not Started**
- Any implementation code, UI design, or wireframes.
- Every Phase 2 (Implementation Planning) Tier C–F document, and every Tier B document beyond the five drafted so far, per `IMPLEMENTATION_PLANNING.md` §4 — none begins without Paul's explicit direction to begin that specific document.
...
- **The local payment provider module's provider choice, cash-on-delivery decision, and refund policy are all undecided** — recorded in `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` §4/§10/§16/§22, none resolved; blocks full implementation of `07_CHECKOUT_SPECIFICATION.md`'s payment-dependent sections once Tier C/development begins, though none blocked this draft.

**Awaiting Paul's Approval** (full list with document references in `docs/PROJECT_STATUS.md`)
- **Review of `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` (v1.0 — Draft)** and either approval for a refinement/freeze pass, or direction to change it — including the newly-discovered `11_ADMIN_WORKFLOWS_SPECIFICATION.md` refund-mechanism documentation gap above.
- Which Tier B module begins next after the local payment provider — see `docs/implementation-planning/MODULE_INVENTORY.md` for the remaining candidates (the notification provider, Saved-for-Later); nothing begins without Paul's direction.
...
- **Phase 0f — Implementation Planning (Paul's "Phase 2"): Tier A complete; four Tier B modules Approved; a fifth Drafted.** `IMPLEMENTATION_PLANNING.md` v1.0 defines the six-tier structure; `TIER_A_FOUNDATIONAL_RECONCILIATION.md` (v1.4), `TIER_B_PRODUCT_RELATIONSHIP_MODULE.md`, `TIER_B_WINE_ATTRIBUTES_MODULE.md`, `TIER_B_FOOD_ATTRIBUTES_MODULE.md`, and `TIER_B_DELIVERY_SLOT_MODULE.md` are all Approved at v1.0; `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` (v1.0 — Draft) awaits Paul's review; `MODULE_INVENTORY.md` (v1.7) is live; no refinement/freeze pass on the local-payment-provider draft, and no further Tier B module, begins without Paul's explicit direction.
...
Phase 1 — Product Specifications is complete (all 11 frozen), Phase 2's Tier A (Foundational Reconciliation) is complete and Approved (v1.4), four Tier B documents — the Product Relationship Module, the Wine & Spirits Attributes Module, the Food Attributes Module, and the Delivery-Slot Module — are Approved at v1.0, and a fifth, the Local Payment Provider Module, is drafted (v1.0 — Draft):

1. **Paul to review `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md` (v1.0 — Draft)** and either approve it for a refinement/freeze pass or direct changes — including the newly-discovered `11_ADMIN_WORKFLOWS_SPECIFICATION.md` refund-mechanism documentation gap (§1, §15, §21).
2. **Per Paul's explicit instruction, no refinement/freeze pass on this draft, and no further Tier B document, begins without his explicit direction** — the same placeholder-then-draft-then-approve discipline that governed all 11 Product Specifications, per `IMPLEMENTATION_PLANNING.md` §7 and `DOCUMENTATION_GOVERNANCE.md` §5/§8.
3. **`docs/implementation-planning/MODULE_INVENTORY.md` lists every remaining not-yet-scoped candidate** for whenever Paul is ready to direct the next Tier B module after the local payment provider (the notification provider, Saved-for-Later).
4. **Two architectural questions from `TIER_B_DELIVERY_SLOT_MODULE.md` remain open**, independent of which module begins next: the pickup-slot mechanism boundary (§5, §19); and the kitchen-capacity-vs-rider-capacity reconciliation question (§6, §18).
5. **Paul still needs to resolve the payment-provider decision itself** (`MEDUSA_EXTENSIONS.md` #4) — the project's sole launch-blocking open item, independent of this module's own architectural approval.
6. **Phase 0c (component specification)** also awaits Paul's direction to begin, and can run in parallel with Phase 2.
...
| Last Updated | 2026-07-19 |
| Project Phase | Phase 0 complete and frozen; Phase 1 — Product Specifications complete (all 11 frozen); Phase 2 — Implementation Planning's Tier A Approved (v1.4), four Tier B modules (Product Relationship Module, Wine & Spirits Attributes Module, Food Attributes Module, Delivery-Slot Module) Approved, a fifth (Local Payment Provider Module) Drafted (v1.0 — Draft) |
| Next Planned Milestone | Paul's review and approval of `TIER_B_LOCAL_PAYMENT_PROVIDER_MODULE.md`; then his direction on which Tier B module begins next (see `MODULE_INVENTORY.md`); the payment-provider decision itself — see Section 10 |
