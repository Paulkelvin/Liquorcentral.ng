...
└── docs/
    ...
    │   ├── TIER_A_FOUNDATIONAL_RECONCILIATION.md      Approved, v1.3 — complete
    ...
    │   ├── TIER_B_DELIVERY_SLOT_MODULE.md             Draft, v1.0 — fourth Tier B module, awaiting review
    │   └── MODULE_INVENTORY.md                        Approved, living, v1.5 — index of every module
...
`/docs` contains 23 documents plus the 11-file `specifications/` directory (34 files total). `docs/README.md` is the detailed document map with cross-links and the document-status convention; `docs/DOCUMENTATION_GOVERNANCE.md` is the governing standard behind that convention (statuses, change rules, cross-reference rules, versioning, contributor rules); this section summarizes current status by category.
...
- `docs/implementation-planning/MODULE_INVENTORY.md` — living, approved 2026-07-18, corrected to v1.5 (2026-07-19). Single-page index of every module/integration/capability identified anywhere in `/docs`, with type, status, dependencies, and launch-criticality.
...
- `docs/implementation-planning/TIER_B_DELIVERY_SLOT_MODULE.md` — the fourth Tier B module, drafted 2026-07-19. Purely architectural definition of the delivery-slot scheduling module — scheduling and capacity concepts, a conceptual time-slot lifecycle, ownership, its relationship to Medusa's native fulfillment flow, and integration points across Checkout/Food Ordering/Delivery/Admin Workflows, plus confirmed non-integrations (Cart among them). Corrected the module's dependency list (`01`,`02`,`07`,`09`,`10`,`11`, not `06`,`07`,`09`,`10`) during drafting. Names, but does not resolve, two architectural questions: the pickup-slot mechanism boundary, and how kitchen-capacity and rider-capacity constraints reconcile into one tracked capacity figure. No database, API, or code content.

### In Progress

- None. No Product Specification is in progress. `TIER_B_DELIVERY_SLOT_MODULE.md` is Draft, not In Progress — it awaits Paul's review, not further authoring.

### Not Started

- No Product Specification placeholders remain — all 11 are frozen. Beyond Tier A (Approved), three Tier B modules Approved, and a fourth Tier B module Drafted (see Completed/Draft, above), no further Phase 2 implementation-planning document (per `IMPLEMENTATION_PLANNING.md` §4) has been started; none begins without Paul's explicit direction to begin that specific document.
...
**In Progress**
- None. `TIER_B_DELIVERY_SLOT_MODULE.md` is Draft, awaiting Paul's review — not further authoring. No further Tier B document begins until Paul approves one.

**Not Started**
- Any implementation code, UI design, or wireframes.
- Every Phase 2 (Implementation Planning) Tier C–F document, and every Tier B document beyond the four drafted so far, per `IMPLEMENTATION_PLANNING.md` §4 — none begins without Paul's explicit direction to begin that specific document.
...
- **The delivery-slot module's operational parameters (slot length, cutoff, capacity), the pickup-slot mechanism boundary, and the kitchen-vs-rider capacity reconciliation question are all undecided** — recorded in `TIER_B_DELIVERY_SLOT_MODULE.md` §9/§18/§19/§21, none resolved; blocks full implementation of this module's six dependent specifications once Tier C/development begins, though none blocked this draft.

**Awaiting Paul's Approval** (full list with document references in `docs/PROJECT_STATUS.md`)
- **Review of `TIER_B_DELIVERY_SLOT_MODULE.md` (v1.0 — Draft)** and either approval for a refinement/freeze pass, or direction to change it — including the two named-but-unresolved architectural questions above.
- Which Tier B module begins next after delivery-slot — see `docs/implementation-planning/MODULE_INVENTORY.md` for the remaining candidates (the local payment provider, the notification provider, Saved-for-Later); nothing begins without Paul's direction.
...
- **Phase 0f — Implementation Planning (Paul's "Phase 2"): Tier A complete; three Tier B modules Approved; a fourth Drafted.** `IMPLEMENTATION_PLANNING.md` v1.0 defines the six-tier structure; `TIER_A_FOUNDATIONAL_RECONCILIATION.md` (v1.3), `TIER_B_PRODUCT_RELATIONSHIP_MODULE.md`, `TIER_B_WINE_ATTRIBUTES_MODULE.md`, and `TIER_B_FOOD_ATTRIBUTES_MODULE.md` are all Approved at v1.0; `TIER_B_DELIVERY_SLOT_MODULE.md` (v1.0 — Draft) awaits Paul's review; `MODULE_INVENTORY.md` (v1.5) is live; no refinement/freeze pass on the delivery-slot draft, and no further Tier B module, begins without Paul's explicit direction.
...
Phase 1 — Product Specifications is complete (all 11 frozen), Phase 2's Tier A (Foundational Reconciliation) is complete and Approved (v1.3), three Tier B documents — the Product Relationship Module, the Wine & Spirits Attributes Module, and the Food Attributes Module — are Approved at v1.0, and a fourth, the Delivery-Slot Module, is drafted (v1.0 — Draft):

1. **Paul to review `TIER_B_DELIVERY_SLOT_MODULE.md` (v1.0 — Draft)** and either approve it for a refinement/freeze pass or direct changes — including two named-but-unresolved architectural questions (the pickup-slot mechanism boundary; the kitchen-vs-rider capacity reconciliation question).
...
3. **`docs/implementation-planning/MODULE_INVENTORY.md` lists every remaining not-yet-scoped candidate** for whenever Paul is ready to direct the next Tier B module after delivery-slot (the local payment provider, the notification provider, Saved-for-Later).
...
| Last Updated | 2026-07-19 |
| Project Phase | Phase 0 complete and frozen; Phase 1 — Product Specifications complete (all 11 frozen); Phase 2 — Implementation Planning's Tier A Approved (v1.3), three Tier B modules (Product Relationship Module, Wine & Spirits Attributes Module, Food Attributes Module) Approved, a fourth (Delivery-Slot Module) Drafted (v1.0 — Draft) |
| Next Planned Milestone | Paul's review and approval of `TIER_B_DELIVERY_SLOT_MODULE.md`; then his direction on which Tier B module begins next (see `MODULE_INVENTORY.md`); the payment-provider decision — see Section 10 |
